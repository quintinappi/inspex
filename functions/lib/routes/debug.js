...
const debugDoorQuery = async (req, res) => {
    ...
    try {
        ...
        const doors = doorsSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
        ...
        if (doors.length > 0) {
            ...
            const inspectionsSnapshot = await db.db.collection('door_inspections')
                ...
                .get();
            relatedData.inspections = inspectionsSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
            // Get certifications
            const certsSnapshot = await db.db.collection('certifications')
                .where('door_id', '==', doorId)
                .get();
            relatedData.certifications = certsSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
        }
        res.json({
...
exports.debugDoorQuery = debugDoorQuery;
// Debug endpoint to get all doors
const debugAllDoors = async (req, res) => {
...
exports.debugAllDoors = debugAllDoors;
